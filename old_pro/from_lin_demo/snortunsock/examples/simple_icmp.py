...
def main():
    protocol={'1':"ICMP",'2':"IGMP",'3':"GGP",'4':"IPv6",'5':"ST",'6':"TCP",'17':"UDP"}
...
#            print(sig_id,sig_rev,sig_classification,priority,event_id,event_reference,ref_time.tv_sec,ref_time.tv_usec)
           ...
           # print(time.gmtime(ref_time.tv_usec))
            ...
            msg=r.match(msg)[1]
            act="alert"
            class1=["NMAP SCAN","UNKNOWN attack","Attempted DDOS","Attempt login","Generic ICMP event","TCP connection"]
        ...
        #          (ip_to_str(ip.src), ip_to_str(ip.dst), ip.len, ip.ttl, do_not_fragment, more_fragments, fragment_offset))
            ...
            final_msg={"sid":sig_id,"priority":priority,"class":a,"iface":"ens192","action":act,"src_addr":ip_to_str(ip.src),"src_port":sport,"dst_addr":ip_to_str(ip.dst),"dst_port":dport,"proto":protocol[str(ip.p)],"timestamp":time1,"msg":msg,"is_attack":classf}
            req=requests.post(url="http://192.168.1.144:8000/api/insertData",data=final_msg)
            print(req.text)
            print(final_msg)
